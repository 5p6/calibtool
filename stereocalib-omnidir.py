...
def main(args:argparse.Namespace):
    ...
    "左相机标定"
    left_ret_l,K1,xi1,D1,rvecs1,tvecs1,idx1=cv2.omnidir.calibrate(objpoints,left_imgpoints,(w,h),K=None,xi=None,D=None,rvecs=None, tvecs=None, flags=flag,criteria=criteria)
    print("左相机误差 : %f"%left_ret_l)
    "右相机标定"
    right_ret_r,K2,xi2,D2,rvecs2,tvecs2,idx2=cv2.omnidir.calibrate(objpoints,right_imgpoints,(w,h),K=None,xi=None,D=None,rvecs=None, tvecs=None, flags=flag,criteria=criteria)
    ...
    flag = 0
    "鱼眼双目标定"
    retval,onj,img_l,img_r,K1,xi1,D1,K2,xi2,D2,rvecs,tvecs,rvecsL,tvecsL,idx= cv2.omnidir.stereoCalibrate(
        objpoints,
        left_imgpoints,
        right_imgpoints,
        imageSize1=(w,h),
    ...
    "开始矫正"
    "左右映射图"
    left_map1,left_map2=cv2.omnidir.initUndistortRectifyMap(K1,D1,xi1,R1,None,(w,h),cv2.CV_32FC1,cv2.omnidir.RECTIFY_PERSPECTIVE)
    # left_map1,left_map2=cv2.fisheye.initUndistortRectifyMap(K1, D1, R1, P1,(w,h),cv2.CV_32FC1)
    right_map1,right_map2=cv2.omnidir.initUndistortRectifyMap(K2,D2,xi2,R2,None,(w,h),cv2.CV_32FC1,cv2.omnidir.RECTIFY_PERSPECTIVE)
...
if __name__=="__main__":
    ...
    ## 图像根目录
    parse.add_argument("-l","--left_root_dir",default="E:\\dataset\\calibrate\\fishpicure\\new_stereo\\left12.8",help="左标定图像根目录")
    parse.add_argument("-r","--right_root_dir",default="E:\\dataset\\calibrate\\fishpicure\\new_stereo\\right12.8",help="右标定图像根目录")
    ## 棋盘格查找和算法的一些选项
    ...
    parse.add_argument("-rs","--radius_size",default=(3,3),type=int,nargs=2,help = "亚像素角点查找半径")
    ## 效果显示的选项
    parse.add_argument("-sd","--showdetail",default=False,action="store_true",help="是否一直显示调试图像")
    ## 参数保存
    parse.add_argument("-so","--save_option",default=False,action="store_true",help= "是否保存标定参数")
    parse.add_argument("-sp","--save_path",default="./param/fisheye.yaml",help="保存参数的路径")
    ## 去畸变示例和保存畸变矫正图像
    parse.add_argument("-ind","--test_index",default=2,type=int,help="展示结果图像的索引")
    parse.add_argument("-sim","--save_img",default=0,type=int,help="是否保存畸变校正的图像")
    ## 图像尺寸重定义,为了适配立体匹配算法
    parse.add_argument("--img_resizeoption",default=False,action="store_true",help="图像是否重定尺度")
    parse.add_argument("--resize_times",default=0.5,type=float,help= "重定尺度倍数")
