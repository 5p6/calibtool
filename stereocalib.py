...
if __name__=="__main__":
    ...
    ## 图像根目录
    parse.add_argument("-l","--left_root_dir",default=None,help="标定左图像的根目录")
    parse.add_argument("-r","--right_root_dir",default=None,help="标定右图像的根目录")
    ## 棋盘格查找和算法的一些选项
    ...
    parse.add_argument("-s","--square_size",default= 0.06 ,type= float,help="棋盘格尺寸(m)")
    parse.add_argument("-rs","--radius_size",default=(7,7),type=int,nargs=2,help = "亚像素角点查找半径")
    ## 效果显示的选项
    parse.add_argument("-sd","--showdetail",default=False,action="store_true",help="是否显示具体内容")
    ## 参数保存
    ...
    ## 去畸变示例和保存畸变矫正图像
    parse.add_argument("-ind","--test_index",default=3,type=int,help="测试校正的图像索引")
    parse.add_argument("-sim","--save_img",default=False,action="store_true",help="是否保存图像")
    ## 图像尺寸重定义,为了适配立体匹配算法
    parse.add_argument("--img_resizeoption",default=False,action="store_true",help="重定尺度标志")
    parse.add_argument("--resize_times",default=0.5,type=float,help= "尺度倍数")
