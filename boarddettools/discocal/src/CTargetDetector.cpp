...
pair<bool, vector<Shape>> TargetDetector::detect(cv::Mat &img, string type)
{
    ...
    if (type == "square")
    {
        ...
        for (int i = 0; i < corners.size(); i++)
            control_shapes.push_back(Shape(corners[i].x, corners[i].y, 0));
...
std::tuple<bool, vector<Shape>, vector<Shape>> TargetDetector::DetectStereoCircle(cv::Mat &left_img, cv::Mat &right_img)
{
    if (this->draw)
    {
        this->drawing_scale = 1000.0 / max(left_img.rows, left_img.cols);
    }
    bool ret;
    vector<Shape> control_shapes;
    ...
    std::tuple<bool, vector<Shape>, vector<Shape>> final_result;

    cv::Mat leftbgr_img, leftoutput_img;
    cv::Mat rightbgr_img, rightoutput_img;

    cv::cvtColor(left_img, leftbgr_img, cv::COLOR_GRAY2BGR);
    cv::cvtColor(right_img, rightbgr_img, cv::COLOR_GRAY2BGR);

    int output_col = left_img.cols * this->drawing_scale;
    int output_row = left_img.rows * this->drawing_scale;

    cv::resize(leftbgr_img, leftoutput_img, cv::Size(output_col, output_row));
    cv::resize(rightbgr_img, rightoutput_img, cv::Size(output_col, output_row));

    // detect circles
    bool leftret = detect_circles(left_img, leftoutput_img, left_shapes, draw);
    bool rightret = detect_circles(right_img, rightoutput_img, right_shapes, draw);

    ret = leftret && rightret;

    final_result = std::make_tuple(ret, left_shapes, right_shapes);

    visualize_result(leftoutput_img, rightoutput_img, final_result);
...
// draw shape on single picure
void TargetDetector::drawshapes(cv::Mat &img_output, const pair<bool, vector<Shape>> &result)
{
    // visualize
    ...
    {
        if (result.first)
            cv::putText(img_output, "save: 1, ignore: 0, quit_visualize: q", cv::Point2f(left, bottom), 0, fontscale, cv::Scalar(0, 0, 255), thickness);
        else
            cv::putText(img_output, "detection fail, press any key", cv::Point2f(left, bottom), 0, fontscale, cv::Scalar(0, 0, 255), thickness);
        this->detection_result = img_output;
    }
    else
    {
        if (!result.first)
            cv::putText(img_output, "detection fail", cv::Point2f(left, bottom), 0, fontscale, cv::Scalar(0, 0, 255), thickness);
        this->detection_result = img_output;
    }
}

// for stereo
void TargetDetector::visualize_result(cv::Mat &left_output, cv::Mat &right_output, std::tuple<bool, vector<Shape>, vector<Shape>> &result)
{

    this->drawshapes(left_output, std::make_pair(std::get<0>(result), std::get<1>(result)));
    this->drawshapes(right_output, std::make_pair(std::get<0>(result), std::get<2>(result)));

    cv::Mat concat;

    cv::hconcat(left_output, right_output, concat);
    cv::imshow("concat image", concat);
...
{
    this->drawshapes(img_output, result);
    cv::imshow("input image", img_output);
    printf("Interact with the visualization window\n");
    char key = cv::waitKey(0);
...
#pragma omp critical
    ...
    if (isnan(c_cov(0, 0)) || isnan(c_cov(1, 0)) || isnan(c_cov(1, 1)))
        return false;
...
bool TargetDetector::detect_circles(cv::Mat &img, cv::Mat &img_output, vector<Shape> &target, bool debug)
{
    ...
    for (int bs : block_sizes)
    {
        ...
        for (int s : kernel_sizes)
        {
            ...
            {
                if (contour.size() < contour_min_length)
                    continue;
