#include "chessboard/TargetDetector.hpp"
...
namespace camodocal
{
    ...
    bool TargetDetector::detect(const cv::Mat &img, std::vector<cv::Point2f>& corners)
    {
        return detect_circles(img,corners,false);
    }
        ...
        if (target.size() == n_x * n_y)
        {
            ...
            if (debug)
            {
                for (int i = 0; i < target.size(); i++)
                {
                    ...
                    {
                        if (i / n_x == j)
                            cv::putText(img_output, to_string(i % n_x), pt, 0, 1, text_colors[j], 2);
                    }
                }
            }
        }
