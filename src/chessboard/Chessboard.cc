...
#include "chessboard/ChessboardQuad.h"
...
namespace camodocal
{
    ...
    Chessboard::Chessboard(cv::Size boardSize, cv::Mat &image, BoardType boardType)
        : mBoardSize(boardSize), mCornersFound(false), mBoardType(boardType),mCircleDetector(boardSize.width,boardSize.height)
    {
        if (image.channels() == 1)
        {
    ...
    {
        if (mBoardType == BoardType::Square)
        {
            std::cout << "the board type : Square" << std::endl;
            if (useOpenCV)
            {
        ...
        else if (mBoardType == BoardType::Circle)
        {
            std::cout << "the board type : Circle" << std::endl;
            if (useOpenCV)
            {
            ...
            {
                return mCircleDetector.detect(image,corners);
            }
        }
    }
