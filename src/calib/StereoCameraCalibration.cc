...
StereoCameraCalibration::calibrate(void)
{
    ...
    if (m_verbose)
    {
        ...
        cameraLeft_errors = m_calibLeft.getimgerror();
        cameraRight_errors = m_calibRight.getimgerror();
        std::cout<<"per errof of left image : "<<std::endl;
        for(int i = 0;i<cameraLeft_errors.size();i++)
            std::cout<< cameraLeft_errors[i]<<" ";
        std::cout<<"\nper errof of right image : "<<std::endl;
        for(int i = 0;i<cameraRight_errors.size();i++)
            std::cout<< cameraRight_errors[i]<<" ";
        std::cout<<std::endl;
...
StereoCameraCalibration::writeParams(const std::string& directory) const
{
    ...
    cv::FileStorage fs(directory + "/camera.yaml", cv::FileStorage::WRITE);
    Eigen::Quaterniond qrotation(m_q.w(),m_q.x(),m_q.y(),m_q.z());
    Eigen::Isometry3d T;
    T.setIdentity();
    T.rotate(qrotation);
    T.translate(m_t);
    std::cout<< T.matrix()<<std::endl;
    cv::Mat Tcv(4,4,CV_64FC1);
    for(int i = 0; i < 4;i++){
        for(int j = 0;j<4;j++)
        Tcv.at<double>(i,j) = T.matrix()(i,j);
    }
    ...
    fs << "{" << "q_x" << m_q.x()
              ...
              << "t_y" << m_t(1)
              << "t_z" << m_t(2) << "}";
    fs << "Transfrom Matrix"<< Tcv.inv();
    fs << "R"<<(cv::Mat_<double>(3,3)<< \
    qrotation.matrix()(0,0),qrotation.matrix()(0,1),qrotation.matrix()(0,2),
    qrotation.matrix()(1,0),qrotation.matrix()(1,1),qrotation.matrix()(1,2),
    qrotation.matrix()(2,0),qrotation.matrix()(2,1),qrotation.matrix()(2,2)
    )
    << "T"<<(cv::Mat_<double>(3,1)<<m_t(0),m_t(1),m_t(2));
    fs.release();
}
