...
namespace camodocal
{
    ...
    CameraCalibration::calibrate(void)
    {
        ...
        // main function , optimize the parameters
        bool ret = calibrateHelper(m_camera, rvecs, tvecs);

        // then calculate the error ,which include covariance matrix from the pixel coordinate and reprojection error.
        ...
        for (size_t i = 0; i < errVec.size(); ++i)
        {
            ...
            for (size_t j = 0; j < errVec.at(i).size(); ++j)
            {
                ...
                error_sum += sqrt(d0 * d0 + d1 * d1);
                measurementCovariance(0, 0) += d0 * d0;
                measurementCovariance(0, 1) += d0 * d1;
                measurementCovariance(1, 1) += d1 * d1;
            }
            // record the error from every image
            m_imgpererror.push_back(error_sum / errVec.at(i).size());
        }
    ...
    std::vector<double> &CameraCalibration::getimgerror(void)
    {
        return m_imgpererror;
    }

    const std::vector<double> &CameraCalibration::getimgerror(void) const
    {
        return m_imgpererror;
    }
}
