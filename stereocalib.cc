...
#include "calib/StereoCameraCalibration.h"
#include "gpl/gpl.h"
...
int main(int argc, char** argv)
{
    ...
    bool viewResults;
    bool verbose;
    std::string boardtypestr;
    camodocal::Chessboard::BoardType boardType;
    ...
    desc.add_options()
        ...
        ("output,o", boost::program_options::value<std::string>(&outputDir)->default_value("./param"), "Output directory containing calibration data")
        ...
        ("camera-model", boost::program_options::value<std::string>(&cameraModel)->default_value("mei"), "Camera model: Kannala-brandt | Mei | Pinhole | Scaramuzza")
        ...
        ("board-type",boost::program_options::value<std::string>(&boardtypestr)->default_value("Square"),"the board type, Square or Circle")
        ;
    ...
    if (boardtypestr == "Square")
    {
        boardType = camodocal::Chessboard::Square;
    }
    else if (boardtypestr == "Circle")
    {
        boardType = camodocal::Chessboard::Circle;
    }
    else{
        throw std::runtime_error("the board type does not satisfy the rquest!");
    }
    ...
    for (size_t i = 0; i < imageFilenamesL.size(); ++i)
    {
        ...
        camodocal::Chessboard chessboardL(boardSize, imageL,boardType);
        camodocal::Chessboard chessboardR(boardSize, imageR,boardType);
